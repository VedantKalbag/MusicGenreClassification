...
def saveFnames(fnames):
    ...
    for i in np.arange(numFnames):
        ...
        if i % 10000 == 0 or i == numFnames-1:
            np.savetxt(f'../datasets/gtzan10sAug/Final/filenames/{j}.txt', names, fmt="%s")
            names = np.array([])
            j += 1
...
def loadDatasetNames():
    ...
    path = f'../datasets/gtzan10sAug/Final/filenames/'
...
if __name__ == '__main__':
    # splitType = 'train'
    savePath = f'../datasets/gtzan10sAug/Final/features/'
